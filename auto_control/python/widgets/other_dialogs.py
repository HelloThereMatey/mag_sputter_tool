...
class LogoutDialog(QDialog):
    """Dialog to confirm logout and optionally shut down the application.
    
    Usage:
        dlg = LogoutDialog(current_username, parent)
        if dlg.exec() == QDialog.Accepted:
            action = dlg.get_logout_action()  # Returns 'logout' or 'shutdown'
    """
    def __init__(self, current_username: str, parent=None):
        ...
        self._logout_action = None  # Will be 'logout' or 'shutdown'
        
        layout = QVBoxLayout()
        layout.setSpacing(20)
        
        # User info label
        user_label = QLabel(f"Current User: {current_username}")
        ...
        # Logout button
        logout_btn = QPushButton("Logout (Return to Login Screen)")
        ...
        # Shutdown button
        shutdown_btn = QPushButton("Logout and Exit Application")
    ...
    def _on_logout(self):
        """Handle logout button - return to login screen."""
        self._logout_action = 'logout'
        self.accept()
    
    def _on_shutdown(self):
        """Handle shutdown button - exit application."""
        reply = QMessageBox.question(
            self,
            "Confirm Exit",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self._logout_action = 'shutdown'
            self.accept()
    
    def get_logout_action(self) -> Optional[str]:
        """Get the selected logout action.
        
        Returns:
            'logout' - Return to login screen
            'shutdown' - Exit application
            None - Dialog was cancelled
        """
        if self.result() == QDialog.Accepted:
